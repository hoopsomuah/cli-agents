...
const ASSET_WEBP = '../site/assets/images/webp/';
const ASSET_DIAGRAM = '../site/assets/diagrams/';
const SRCSET_WIDTHS = [768, 1280, 1920];

// Deck slide images fill the right half of a 16:9 frame. On most projectors
// that's ~1280 CSS px wide; the browser still picks an appropriate variant.
const DECK_SIZES = '(max-width: 900px) 90vw, (max-width: 1600px) 50vw, 1280px';

// Emit a <picture> with WebP <source> + original <img> fallback. The WebP
// variants are generated by scripts/generate-webp.py and committed under
// site/assets/images/webp/<width>/. If the WebP files are missing, the <img>
// fallback still works.
function responsivePicture(filename, alt, sizes) {
  if (!filename) return '';
  const stem = String(filename).replace(/\.(png|jpe?g)$/i, '');
  const srcset = SRCSET_WIDTHS
    .map((w) => `${ASSET_WEBP}${w}/${stem}.webp ${w}w`)
    .join(', ');
  return `
              <picture>
                <source type="image/webp" srcset="${srcset}" sizes="${sizes}" />
                <img src="${ASSET_IMG}${filename}" alt="${escapeHtml(alt || '')}" decoding="async" />
              </picture>`;
}
