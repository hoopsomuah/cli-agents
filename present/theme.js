// theme.js — palette + mode picker (presentation deck)
//
// Mirrors site/js/theme.js but with two differences:
//   - default mode is "dark" (vs "light" for the reading view)
//   - picker open state is published on document.documentElement
//     so deck.js can pause keyboard navigation while picking
...
function setPickerOpenFlag(isOpen) {
  // Surface a flag on the root so deck.js can suppress keyboard nav.
  document.documentElement.toggleAttribute('data-picker-open', isOpen);
}
...
function init() {
  ...
  if (panel) {
    ...
    panel.addEventListener('keydown', (e) => {
      // Stop deck navigation arrows/space while picking.
      e.stopPropagation();
      if (e.key === 'Escape') {
        closePicker();
