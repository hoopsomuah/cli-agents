...
// Responsive <picture> emitter. Pairs every <img> with a WebP <source> at
// three widths (768/1280/1920). The WebP variants are generated by
// scripts/generate-webp.py and committed under site/assets/images/webp/<w>/.
// If the WebP files are missing the <img> fallback still works.
function renderResponsiveImage(filename, alt, sizes, extraClass) {
  if (!filename) return '';
  // Strip extension to get the WebP stem (works for .png/.jpg/.jpeg).
  const stem = filename.replace(/\.(png|jpe?g)$/i, '');
  const srcset = [768, 1280, 1920]
    .map((w) => `site/assets/images/webp/${w}/${stem}.webp ${w}w`)
    .join(', ');
  const cls = extraClass ? ` class="${extraClass}"` : '';
  return `<picture${cls}>
      <source type="image/webp" srcset="${srcset}" sizes="${escapeAttr(sizes)}" />
      <img src="site/assets/images/${filename}" alt="${escapeAttr(alt || '')}" loading="lazy" decoding="async" />
    </picture>`;
}
